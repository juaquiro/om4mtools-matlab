classdef test_PUMexLib < matlab.unittest.TestCase
    %run(test_PUMexLib)
    ...
    methods(TestMethodSetup)
        function SetUp(testCase)
            ...
            testCase.Lib='PUMexLib';
            testCase.LibH=[testCase.Lib, '.h'];
            
            if not(libisloaded(testCase.Lib))
                %load the library with alias 'lib'
                [notfound,warnings]=loadlibrary(testCase.Lib, testCase.LibH);
                libfunctions(testCase.Lib, '-full')
                fprintf('\n')
            end
    ...
    methods(TestMethodTeardown)
        function TearDown(testCase)
            ...
            %unload library
            if libisloaded(testCase.Lib)
                unloadlibrary(testCase.Lib);
            end
    ...
    methods (Test)
        function test_PUFlynMdMex(testCase)
            %run(test_PUMexLib, 'test_PUFlynMdMex')
            ...
            calllib(testCase.Lib, 'PUFlynMdMex', pw, mask, qual, unw, thresh_flag, fatten);
