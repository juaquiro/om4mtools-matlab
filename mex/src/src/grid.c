   ...
   float *dxwts, float *dywts, int w, int h, int numit, int mindim)
{
  ...
  if (!Coarsest(w, h, mindim)) {
	...
	// AQ CAMBIO Allocate no inicializa a cero en MSVC
	Zero(dxwts2, w2, h2);
	Zero(dywts2, w2, h2); 
                   ...
                   float *fine, int wf, int hf) 
{
  ...
  for (j=0; j<hc; j++) {
    ...
    for (i=0; i<wc; i++) {
      ...
      if (f[k5]==0.0 && f[k9]==0.0) c[j*wc + i] = 0.0f;
      else if (f[k6]==0.0 && f[k8]==0.0) c[j*wc + i] = 0.0f;
      else if (f[k6]==0.0 && f[k9]==0.0) c[j*wc + i] = 0.0f;
      else if (f[k5]==0.0 && f[k8]==0.0) c[j*wc + i] = 0.0f;
      else if ((i==0 || c[j*wc + i - 1] != 0.0) &&
        ((f[k4]==0.0 && f[k8]==0.0) 
          ||  (f[k5]==0.0f && f[k7]==0.0f)))
                                     c[j*wc + i] = 0.0f;
      else c[j*wc + i] = 0.25f*(f[k5] + f[k6] + f[k8] + f[k9]);
    }
  }
}
                   ...
                   float *fine, int wf, int hf) 
{
  ...
  for (j=0; j<hc; j++) {
    ...
    for (i=0; i<wc; i++) {
      ...
      if (f[k5]==0.0f && f[k9]==0.0f) c[j*wc + i] = 0.0f;
      else if (f[k6]==0.0f && f[k8]==0.0f) c[j*wc + i] = 0.0f;
      else if (f[k8]==0.0f && f[k9]==0.0f) c[j*wc + i] = 0.0f;
      else if (f[k5]==0.0f && f[k6]==0.0f) c[j*wc + i] = 0.0f;
      else if ((j==0 || c[(j - 1)*wc + i] != 0.0f) &&
          ((f[k2]==0.0f && f[k6]==0.0f)
            ||  (f[k3]==0.0f && f[k5]==0.0f)))
                                     c[j*wc + i] = 0.0f;
      else c[j*wc + i] = 0.25f*(f[k5] + f[k6] + f[k8] + f[k9]);
