  ...
  for (j=0, k=0; j<hc; j++) {
    for (i=0; i<wc; i++, k++) {
      ...
      if (coarse_dxwts && coarse_dywts) {
        ...
        y1 = x1;
        w = w1 + w2;
        y2 = (w > 0.0f) ? (w1*x1 + w2*x2)/w : 0.5f*(x1 + x2);
        w = w1 + w3;
        y3 = (w > 0.0f) ? (w1*x1 + w3*x3)/w : 0.5f*(x1 + x3);
        w = w1 + w2 + w3 + w4;
        y4 = (w > 0.0f) ? (w1*x1 + w2*x2 + w3*x3 + w4*x4)/w
                                  : 0.25f*(x1 + x2 + x3 + x4);
      }
      else {
        y1 = x1;                    y2 = 0.5f*(x1 + x2);
        y3 = 0.5f*(x1 + x3);        y4 = 0.25f*(x1 + x2 + x3 + x4);
      }
      fine[c] += y1;
      fine[c + 1] += y2;
      fine[c + wf] += y3;
              ...
              float *soln, int wf, int hf) 
{
  ...
  for (j=0; j<hc; j++) {
    ...
    for (i=0; i<wc-1; i++) {   /* note: a < wf - 1 */
      ...
      if (dxwts && dywts) {
        if (wmult > 1.0e-6) dx2[j*wc + i] = (float)(scale*(0.25*f5 
                 +  0.125*(f4 + f6 + f2 + f8)
                        +  0.0625*(f1 + f3 + f7 + f9))/wmult);
        else dx2[j*wc + i] = dx[k];
      }
      else {
        dx2[j*wc + i] = (float)(scale*(0.25*f5 + 0.125*(f4 + f6 + f2 + f8)
                                   +  0.0625*(f1 + f3 + f7 + f9)));
      }
    }
  }
  ...
  for (j=1; j<hc; j++) {   /* note: b > 0 */
    ...
    for (i=0; i<wc; i++) {
      ...
      if (dxwts && dywts) {
        if (wmult > 1.0e-6) dy2[j*wc + i] = (float)(scale*(0.25*f5 
                 +  0.125*(f4 + f6 + f2 + f8)
                        +  0.0625*(f1 + f3 + f7 + f9))/wmult);
        else dy2[j*wc + i] = scale*dy[k];
      }
      else {
        dy2[j*wc + i] = (float)(scale*(0.25*f5 + 0.125*(f4 + f6 + f2 + f8)
                                   +  0.0625*(f1 + f3 + f7 + f9)));
      }
    }
  }
