classdef testTempAnalysisRetarExposure < matlab.unittest.TestCase
    % testTempAnalysisRetarExposure explores the temporal demodulation
    % of the retardation vs exposure response, looking for the best
    % demodulation/unwrapping procedure that stays correct despite the
    % low-modulation zones produced by a fast, sudden retardation change
    % near the end of the retardation curve delta(H).
    ...
    methods (Test)
        function testPlotDeltaH4AllCells(testCase)
            % testPlotDeltaH4AllCells plots retardation-vs-thickness
            % curves (delta(H)/thickness) for every cell in the
            % deltaH-12-Jan-2017.mat fixture (generated by the legacy
            % om4mmatlabutils testGetFFTRetarExposure4AllCells)
