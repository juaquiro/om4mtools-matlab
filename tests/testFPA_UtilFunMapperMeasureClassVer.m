classdef testFPA_UtilFunMapperMeasureClassVer < matlab.unittest.TestCase
    % testFPA_UtilFunMapperMeasureClassVer tests LensMapperMeasurement
    % (FFV/Moire deflectometry lens power calculation and calibration),
    % mostly against real acquired lens measurements.
    % Converted from the legacy mtest-framework tests (dropped after
    % R2016a) to matlab.unittest - see LensMapperMeasurement.m.
    ...
    methods (Test)
        ...
        function testGenMapperMeasurement(testCase)
            % testGenMapperMeasurement checks LensMapperMeasurement
            % constructs
        ...
        function testLensMapperMeasurementSaveLoad(testCase)
            % testLensMapperMeasurementSaveLoad fills every
            % LensMapperMeasurement property (except DerSign) and checks
            % save/load round-trips them unchanged, both with the
        ...
        function testGetPowerProgHoya1(testCase)
            % testGetPowerProgHoya1 demodulates a real reference/lens
            % FFV Moire image pair (ProgHoya1*.tif) via FFT sidelobe
            % demodulation and feeds the deflection phasors into
        ...
        function testGetPowerProgHoya(testCase)
            % testGetPowerProgHoya repeats testGetPowerProgHoya1 on a
            % different real reference/lens image pair (ProgHoya*.tif,
            % flat-top filter, extra phasor smoothing)
        ...
        function testGetPowerLente3D_8D(testCase)
            % testGetPowerLente3D_8D repeats testGetPowerProgHoya1's
            % demodulate-and-CalculateLensPower flow on a real 8D lens
            % measurement (Lente3D_8D.bmp), for visual inspection
        ...
        function testGetPowerLinearPal(testCase)
            % testGetPowerLinearPal demodulates a linear-design
            % progressive-lens simulation (LinearPal.mat, generated by
            % GenerateProgressive) and checks CalculateLensPower's S/C/Seq
            % maps against the simulation's theoretical values within tolerance
        ...
        function testGetPowerErfPal(testCase)
            % testGetPowerErfPal repeats testGetPowerLinearPal on an
            % erf-profile progressive-lens simulation (ErfPal.mat) and
            % checks the same S/C/Seq maps against theoretical values
        ...
        function testGetPower_SwissCoat40L88031L(testCase)
            % testGetPower_SwissCoat40L88031L demodulates a real FFV
            % measurement of lens SwissCoat40L88031L against a shared
            % reference image, applying a saved calibration K (or K=[1 1]
            % if none saved) via CalculateLensPower's noRefMethod option,
            % for visual inspection of the power maps
        ...
        function testGetPower_SwissCoat40L88051R(testCase)
            % testGetPower_SwissCoat40L88051R repeats
            % testGetPower_SwissCoat40L88031L for lens
            % SwissCoat40L88051R, using an image-difference-based ROI
            % mask instead of a fixed threshold
        ...
        function testGetPower_YO_D75_SMinus275_C0(testCase)
            % testGetPower_YO_D75_SMinus275_C0 demodulates a real -2.75D
            % lens FFV measurement; if CalibrationFFV.mat does not yet
            % exist it derives and saves a K calibration factor from
            % this lens' known nominal power first, then computes and
            % visually inspects the power maps. If a legacy DLM
            % (LoadMapperFile.m) is available on the path, also loads
            % that tool's own measurement/theoretical/error maps for comparison
