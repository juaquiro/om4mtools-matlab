...
main (int argc, char *argv[])
{
  ...
  mode = (UnwrapMode)SetQualityMode(modekey, qualfile, 0);
  if (mode < 0) exit(BAD_PARAMETER);  /* error msg already printed */

  /*  OPEN FILES, ALLOCATE MEMORY   */
  OpenFile(&ifp, infile, "r+b");
  OpenFile(&ofp, outfile, "w+b");
  if (!Keyword(bmaskfile, "none"))
    OpenFile(&mfp, bmaskfile, "r+b");
  if (mode==corr_coeffs) 
    OpenFile(&qfp, qualfile, "r+b");
  ...
  for (k=0; k<xsize*ysize; k++) {
    if (bitflags[k] & BORDER) qual_map[k] = 0.0f;
  }
  free(bitflags);
  ...
  for (k=0; k<xsize*ysize; k++)  soln[k] = 0.0f;
  ...
  for (k=0; k<xsize*ysize; k++) soln[k] *= (float)TWOPI;
