               ...
               unsigned char *ignore, int ignore_code)
{
  ...
  for (num=0, i=0; i<NUM_BINS; i++) { /* find number of pixels */
    num += histo[i];
  }
  for (sum=0, i=0; i<NUM_BINS; i++) {
    sum += histo[i];
  ...
  for (j=0; j<xsize*ysize; j++) {
    ...
    if (qual[j] < first_val)
      ...
      qual[j] = (float)((qual[j] - first_val)*rmult);
  }
  ...
  if (thresh==0.0) {
    ...
    while (num_coarse_bins <= 100) {
      for (k=0; k<num_coarse_bins; k++)
        coarse_histo[k] = 0.0;
      for (i=0, k=0, sum=0; i<NUM_BINS; i++) {
  ...
  for (j=0, num_zero=0, num=0; j<xsize*ysize; j++) {
    ...
    qual[j] = (qual[j]<thresh) ? 0.0f : 1.0f;
  }
               ...
               int num_bins, unsigned char *ignore, int ignore_code)
{
  ...
  for (j=0, num=0; j<xsize*ysize; j++) {
    ...
    if (qual[j] < 0.0f) qual[j] = 0.0f;
    if (qual[j] > 1.0f) qual[j] = 1.0f;
